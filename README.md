...
- A table of the **top 20 BUY signals** (ranked by ADX / trend strength) —
  signals with price drift beyond `STALE_DRIFT_THRESHOLD_PCT` (default 15%)
  from the actual flip price are excluded from this top-20 view (still
  visible in the full CSV) since they're often already-failed opportunities
  by the time you'd see them.
- A table of the **top 20 SELL / trend-break signals**
- A table of up to **20 Pullback candidates** — a separate, independent
  entry style: RSI oversold within a confirmed uptrend, well off the
  52-week high. Evaluate this separately from the BUY table, not as a
  combined signal.
- **Paper-trade win rate / avg return**, once at least 5 trades have
  actually closed — tracked from the real signal price/date via
  `paper_trades.csv`, which the workflow commits back to the repo after
  every run so the stats accumulate over time instead of resetting daily.
- Small inline charts for up to 10 of the top BUY signals
- The full, untrimmed results (every signal, not just the top 20, including
  stale ones) are attached to the **workflow run itself** as a downloadable
  artifact — go to Actions → the specific run → Artifacts, to grab the
  complete CSV.

Adjust `TOP_N_BUY` / `TOP_N_SELL` / `TOP_N_PULLBACK` near the top of
`nse_scanner.py` if you want different cutoffs.
